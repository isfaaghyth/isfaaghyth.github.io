...
Example: If you go to [isfaa.js.org](/), you end up on my home page. If you go to [isfaa.js.org/me](/me), you're redirected to my about me page. If you go to [isfaa.js.org/categories/javascript](/categories/javascript) or [isfaa.js.org/categories/css](/categories/css), you end up on a category listing page. The routes for these pages look something like this:
...
- [View Source](https://github.com/isfaaghyth/router-example)

#### Prerequisites

- Read [Getting Started with React](https://www.isfaa.js.org/getting-started-with-react/) or [Build a React App with Hooks](https://www.isfaa.js.org/crud-app-in-react-with-hooks/) if you don't know React or React Hooks yet.
- Read [How to Connect to an API in JavaScript](https://www.isfaa.js.org/how-to-connect-to-an-api-with-javascript/) if you don't know how to work with APIs at all.
...
```terminal
Creating a new React app in /Users/isfaaghyth/dev/sandbox/router-example.
...
This would ensure that `isfaa.js.org/categories` would go to a page listing all categories, but `isfaa.js.org/categories/javascript` would go to a completely different template for the individual category listing.
...
export default function HomePage() {
  return (
    <div className="container">
      <h1>Home Page</h1>
      <p>
        <Link to="/isfaaghyth">isfaaghyth</Link> on GitHub.
      </p>
    </div>
  )
}
...
Our `Link` is navigating to `/isfaaghyth`, which will match the `/:id` parameter in `Route`. In order to dynamically get the content from the URL - in this case, `isfaaghyth` - we'll use `match.params.id` from the `props`.

I'm going to use that parameter to make a call to the GitHub API and retrieve my data. In this example I'll be using Hooks, so if you're not familiar with them, please read [Building a CRUD App with Hooks](https://www.isfaa.js.org/crud-app-in-react-with-hooks/).
