...
See the Pen [yYrXRG](http://codepen.io/isfaaghyth/pen/yYrXRG/).

## Additional Styles

## Close button

The close button is `fixed`, with a `z-index` of 3, just to ensure it always appears above the overlay. The `span` setup is the same as from [the responsive dropdown navigation bar](http://isfaa.js.org/responsive-dropdown-navigation-bar) tutorial, except it's not animated. I find this method easier than loading a font icon set or using a unicode letter.
...
- [View demo](http://codepen.io/isfaaghyth/pen/yYrXRG)
