...
var firstName = 'isfaaghyth' // Variable is declared and assigned a value.
```

> **Syntax:** A single equals sign `=` is used to assign a value. In the above example, the value `"isfaaghyth"` is assigned to the variable `firstName`. A semi-colon `;` ends a JavaScript statement, like a period ends a sentence.
...
In the above example, `person` is the **object**, `name` is the **key** (property name), and `"isfaaghyth"` is the **value** (property value).
...
> See the Pen [JS Day One: Echoing a Variable](http://codepen.io/isfaaghyth/pen/VazVPM/) by isfaaghyth ([@isfaaghyth](http://codepen.io/isfaaghyth)) on [CodePen](http://codepen.io).
